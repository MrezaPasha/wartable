...
var CustomJs = function () {
    ...
    var runAjaxSearch = function () {
        var createSearchQuery = function (_this) {
            var name = _this.attr("ir-search-field-name");
            var opr = _this.attr("ir-search-field-opr");
            var tp = _this.attr("ir-search-field-tp");
            var nosp = _this.attr("ir-search-field-noSp");
            if (nosp === undefined) {
                nosp = false;
            }
            var index = ap.list.length;
            var value = _this.val();
            $.each(ap.list, function (i, v) {
                if (v.n === name && v.o === opr) {
                    index = i;
                }
            });
            if (opr === 'in') {
                if (index === ap.list.length) {
                    ap.list[index] = {n: name, v: [value], o: opr, t: tp, s: nosp};
                } else {
                    var invar = ap.list[index].v;
                    var isExist = false;
                    $.each(invar, function (ii, vv) {
                        if (vv === value) {
                            ap.list[index].v.splice(ii, 1);
                            if (ap.list[index].v.length === 0) {
                                ap.list.splice(index, 1);
                            }
                            isExist = true;
                        }
                    });
                    if (!isExist) {
                        ap.list[index].v[ap.list[index].v.length] = value;
                    }
                }
            } else {
                if (value === '' && index !== ap.list.length) {
                    ap.list.splice(index, 1);
                } else if (_this.is(':checkbox') && _this.is(':checked') === false) {
                    ap.list.splice(index, 1);
                } else if (value !== "") {
                    ap.list[index] = {n: name, v: value, o: opr, t: tp, s: nosp};
                }
            }
            if (_this.attr("ir-search-submit") !== undefined) {
                submitSearch();
            }
        };
        ...
        /** AJAX  **/
        var sxContainer = $("[ir-search-container]");
        var fillPageSize = function (paging) {
            var psElm = $("[ir-search-paging-size]");
            if (psElm.html() === undefined) {
                psElm = $("[ir-search-paging-size-template]").clone();
                psElm.removeAttr("ir-search-paging-size-template")
                    .attr("ir-search-paging-size", "");
            }
            sxContainer.append(psElm);
        };
        var fillEmptyBody = function () {
            sxContainer.html("");

            var embElm = $("[ir-search-empty-body-template]").clone();
            embElm.removeAttr("ir-search-empty-body-template")
                .attr("ir-search-empty-body", "");
            sxContainer.append(embElm);
        };
        var clearEmptyBody = function () {
            sxContainer.find("[ir-search-empty-body]").remove();
        };
        var fillErrorBody = function () {
            sxContainer.html("");

            var embElm = $("[ir-search-error-body-template]").clone();
            embElm.removeAttr("ir-search-error-body-template")
                .attr("ir-search-error-body", "");
            sxContainer.append(embElm);
        };
        var clearErrorBody = function () {
            sxContainer.find("[ir-search-error-body]").remove();
        };
        var fillBody = function (list) {
            sxContainer.find("[ir-search-body]").remove();
            var bodyElm = $("[ir-search-body-template]").clone();
            var bodyElmList = bodyElm.find("[ir-search-body-item]").parent();
            bodyElm.removeAttr("ir-search-body-template")
                .attr("ir-search-body", "");
            ...
            var itemTxt = bodyElmList.html();
            itemTxt = itemTxt.replace(new RegExp("ir-search-img-src", "g"), "src");
            var items = "";
            var newItem;
            $.each(list, function (i, value) {
                ...
                $.each(value, function (key, element) {
                    if ($.type(element) === "object") {
                        newItem = replaceObjectVals(key, element, newItem);
                    }
            ...
            bodyElmList.html(items);
            sxContainer.append(bodyElm);
        ...
        var replaceObjectVals = function (ke, elem, nItem) {
            $.each(elem, function (key, element) {
                if ($.type(element) === "object") {
                    nItem = replaceObjectVals(ke + "." + key, element, nItem);
                }
                nItem = nItem.replace(new RegExp("@{" + ke + "." + key + "}", "g"), element);
            });
            return nItem;
        };
        var fillPaging = function (paging) {
            var pagingElm;
            if ($("[ir-search-paging]").html() !== undefined) {
                pagingElm = $("[ir-search-paging]");
                pagingElm.html($("[ir-search-paging-template]").html());
            } else {
                pagingElm = $("[ir-search-paging-template]").clone();
                pagingElm.removeAttr("ir-search-paging-template")
                    .removeClass("hidden")
                    .attr("ir-search-paging", "");
            }
            pagingElm.find("[ir-search-paging-prev]").attr("ir-search-paging-value", paging.index - 1)
                .addClass(paging.index === 1 ? "disable" : "");
            var nxElm = pagingElm.find("[ir-search-paging-next]");
            nxElm.attr("ir-search-paging-value", paging.index + 1)
                .addClass(paging.index === paging.pageCount ? "disable" : "");

            var pItemElm = pagingElm.find("[ir-search-paging-item]").clone();
            pagingElm.find("[ir-search-paging-item]").remove();
            $.each(paging.pages, function (i, v) {
                var itmElm = pItemElm.clone();
                itmElm.attr('ir-search-paging-value', v);
                itmElm.find('span').html(v);
                if (v === paging.index) {
                    itmElm.addClass("active");
                } else {
                    itmElm.removeClass("active");
                }
                itmElm.insertBefore(nxElm);
            });
            pagingElm.find("[ir-search-paging-info-from]").html(paging.from);
            pagingElm.find("[ir-search-paging-info-to]").html(paging.to);
            pagingElm.find("[ir-search-paging-info-count]").html(paging.searchCount);
            sxContainer.append(pagingElm);
        };
        var fillColumnSorting = function () {
            if (ap.o !== undefined && ap.o.length > 0) {
                $("[ir-search-order-col]").removeClass("sorted-desc").removeClass("sorted-asc");
                if (ap.o[0][1] === -1) {
                    $("[ir-search-order-col='" + ap.o[0][0] + "']").addClass("sorted-desc");
                } else {
                    $("[ir-search-order-col='" + ap.o[0][0] + "']").addClass("sorted-asc");
                }
            }
        }

        var searchContainer = $("[ir-search-url]");

        var pageSize = $("[ir-search-paging-size-template] select>option:selected");
        if (pageSize !== undefined && pageSize.val() !== undefined && pageSize.val() !== 0) {
            pageSize = pageSize.val();
        } else {
            var pageSize = searchContainer.attr("ir-search-page-size");
            if (pageSize === undefined || pageSize === 0) {
                pageSize = 10;
            }
        }
        var pageRange = $("[ir-search-paging-template]").attr("ir-search-paging-template");
        try {
            pageRange = Number(pageRange);
        } catch (e) {
            pageRange = 2;
        }
        if (pageRange == undefined || pageRange === 0) {
            pageRange = 2;
        }
        var defaultFill = true;
        if (searchContainer.attr("ir-search-default-fill") == "false") {
            defaultFill = false;
        }
        var ap = {
            c: "",
        ...
        var colsWidth = [];
        var submitSearch = function (submitValues) {
            $.ajax({
                url: searchContainer.attr("ir-search-url") + (submitValues != undefined ? "?submit=" + submitValues : ""),
                Type: 'json',
                type: 'POST',
                data: "ap=" + JSON.stringify(ap),
                success: function (myjson, status, xhr) {
                    if (xhr.getResponseHeader("file-path") != null) {
                        window.open(
                            xhr.getResponseHeader("file-path"),
                            '_blank'
                        );
                        return;
                    }
                    ...
                    clearEmptyBody();
                    clearErrorBody();
                    fillPageSize(myjson.paging);
                    fillBody(myjson.list);
                    fillPaging(myjson.paging);
                    fillColumnSorting();
                },
                error: function () {
                    fillErrorBody();
        ...
        if ($("[ir-search-default-query]").length > 0) {
            $("[ir-search-default-query]").each(function (ix) {
                createSearchQuery($(this));
            });
        }
        if (searchContainer.attr("ir-search-auto-submit") === "true") {
            submitSearch();
        }
        $(document).on('click', "[ir-search-paging-value]", function () {
            if (!$(this).hasClass("disable")) {
                ap.pi = $(this).attr("ir-search-paging-value");
                submitSearch();
            }
        });
        $(document).on('change', "[ir-search-paging-size] select", function () {
            ap.ps = $(this).val();
            submitSearch();
        });
        $(document).on('change', "[ir-search-order-select]", function () {
            ap.o[0] = [$(this).find('option:selected').attr("ir-search-field-name"), $(this).find('option:selected').attr("ir-search-field-order")];
        });
        $(document).on('click', "[ir-search-order-col]", function () {
            var v = $(this).attr("ir-search-order-col");
            if (ap.o[0] !== undefined && ap.o[0][0] === v) {
                ap.o[0] = [v, -ap.o[0][1]];
            } else {
                ap.o[0] = [v, 1];
            }
        ...
        $(document).on('click', "button[ir-search-submit]", function () {
            submitSearch($(this).attr("ir-search-submit"));
        });
        $(document).on('click', "a[ir-search-action-trash]", function () {
            var bu = $(this).data("back");
            var url = $(this).data("trash-url");
            var prefix = "";
            var did = $(this).attr("ir-search-action-trash");
            for (i = 0; i < bu; i++) {
                prefix += "../";
            }
            bootbox.confirm("آیا از حذف اطلاعات " + "<b style='color:red'>" + $(this).data("title") + "</b>" + " با شناسه " + "<b style='color:red'>" + did + "</b>" + " اطمینان دارید؟", function (result) {
                if (result) {
                    $.ajax({
                        url: prefix + (url !== undefined ? url + "/" : 'trash/') + did,
                        async: false
                    }).done(function () {
                        $(this).addClass("done");
                    });
        ...
        $(document).on('change', "[ir-search-field-name]", function () {
            createSearchQuery($(this));
        });
        $(document).on('keyup', '[ir-search-field-name]', function (e) {
            if (e.keyCode === 13) {
                createSearchQuery($(this));
                submitSearch();
                ...
                createSearchQuery($(this));
                submitSearch();
            } else if ($(this).attr("ir-search-submit-realTime") !== undefined) {
                createSearchQuery($(this));
                submitSearch();
            }
        });
    ...
    var runNumberConvertor = function () {
        persianToEnglishNumber = function (value) {
            var newValue = "";
        ...
        englishToPersianNumber = function (value) {
            var newValue = "";
            for (var i = 0; i < value.length; i++) {
                var ch = value.charCodeAt(i);
                if (ch >= 48 && ch <= 57) {
