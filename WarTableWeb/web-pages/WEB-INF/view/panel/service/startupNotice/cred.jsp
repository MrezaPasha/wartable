...
<div class="container">
    <c:choose>
        <c:when test="${startupNotice.id!=0}">
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.startupNotice.edit"/></span>
                <span class="cred-name">[${startupNotice.title}]</span>
            </h3>
        </c:when>
        <c:otherwise>
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.startupNotice.create"/></span>
            </h3>
        </c:otherwise>
    </c:choose>
    <div class="tabbable">
        ...
        <div class="tab-content">
            <form id="form" accept-charset="UTF-8" action="${action}" method="POST">
                <form:hidden id="startupNotice_id" path="startupNotice.id"/>
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <c:set var="varName" value="startupNotice.title"/>
                    ...
                    <div class="col-sm-8">
                        <div class="">
                            <c:set var="varName" value="startupNotice.receivers"/>
                            <form:label path="${varName}" cssClass="control-label">
                                <spring:message code="${varName}"/>
                            </form:label>
                            <form:select multiple="multiple" path="startupNotice.receiverTemps" cssClass="form-control height-auto search-select">
                                <form:options items="${startupNotice.receiverTemps}" itemValue="id" itemLabel="fullName"/>
                                <form:options items="${sulist}" itemValue="id" itemLabel="fullName"/>
                            </form:select>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="">
                            <c:set var="varName" value="startupNotice.messageBody"/>
                            <form:label path="${varName}" cssClass="control-label">
                                <spring:message code="${varName}"/>
                                <span class="symbol required"></span>
                                <form:errors cssClass="form-validation-error" path="${varName}"/>
                            </form:label>
                            <form:textarea rows="10" path="${varName}" cssClass="form-control string-required"/>
                        </div>
                    </div>
                </div>
            </form>
