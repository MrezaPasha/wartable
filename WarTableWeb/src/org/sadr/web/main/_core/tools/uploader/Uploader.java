...
//            byte[] bytes = sourceFile.getBytes();
    ...
    //------------  dropzone
    public ResponseEntity uploadFromDropZone(MultipartHttpServletRequest request, TtRepoDirectory repoDirectory, TtThumbnailsOf thumbnailsOf) {
        return uploadFromDropZone(request, repoDirectory, thumbnailsOf.getUploadIfExist(), thumbnailsOf.getUploadOrginalFile(), thumbnailsOf.getMethod(), thumbnailsOf.getMode(), thumbnailsOf.getCropMode(), thumbnailsOf.getImageOp(), thumbnailsOf.getThumbs());
    }

    public ResponseEntity uploadFromDropZone(MultipartHttpServletRequest request, TtThumbnailsOf thumbnailsOf) {
        return uploadFromDropZone(request, thumbnailsOf.getRepoDirectory(), thumbnailsOf.getUploadIfExist(), thumbnailsOf.getUploadOrginalFile(), thumbnailsOf.getMethod(), thumbnailsOf.getMode(), thumbnailsOf.getCropMode(), thumbnailsOf.getImageOp(), thumbnailsOf.getThumbs());
    }

    public ResponseEntity uploadFromDropZone(MultipartHttpServletRequest request, TtRepoDirectory repoDirectory, TtUploadIfExist ifExist, TtUploadOrginalFile tuof, Scalr.Method method, Scalr.Mode mode, TtThumnailCropMode cropMode, BufferedImageOp op, TtThumbnailSize... sizes) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                MultipartFile mufile = request.getFile(uploadedFile);
                if (sizes == null) {
                    file = upload(mufile, repoDirectory, ifExist, tuof, method, mode, cropMode, op, TtThumbnailSize._150X150);
                } else {
                    TtThumbnailSize[] tts = new TtThumbnailSize[sizes.length + 1];
                    for (int i = 0; i < sizes.length; i++) {
                        tts[i] = sizes[i];
                    }
                    tts[sizes.length] = TtThumbnailSize._150X150;
                    file = upload(mufile, repoDirectory, ifExist, tuof, method, mode, cropMode, op, tts);
                }
                if (file == null) {
                    return new ResponseEntity<>("{}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
                }
                return new ResponseEntity<>(file.getInfoForUploaderJson(), headers, HttpStatus.OK);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>("{}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity uploadFromDropZone(MultipartHttpServletRequest request, String destinationPath) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        try {
            Iterator<String> itr = request.getFileNames();
            while (itr.hasNext()) {
                String uploadedFile = itr.next();
                MultipartFile mufile = request.getFile(uploadedFile);
                if (upload(mufile, destinationPath)) {
    ...
    public Set<File> compeleteUploadFromDropZone(String filesTail) {
        //  OutLog.pl(filesTail);
        if (filesTail != null && !filesTail.isEmpty()) {
            String s[] = filesTail.split(",");
            Set<File> fls = new HashSet<>();
            for (String fi : s) {
                try {
                    File file = this.fileService.findById(Integer.parseInt(fi));
                    if (file != null) {
                        //  OutLog.pl(file.getId());
    ...
    public Set<File> compeleteUploadFromDropZone(String filesTail, String dirPath) {
        //  OutLog.pl(filesTail);
        if (filesTail != null && !filesTail.isEmpty()) {
            String s[] = filesTail.split(",");
            Set<File> fls = new HashSet<>();
            Directory d = this.directoryService.getDirectory(dirPath);
            for (String fi : s) {
                try {
                    File file = this.fileService.findById(Integer.parseInt(fi));
                    if (file != null) {
                        if (file.getIsTemporary()) {
                            file.setIsTemporary(false);
                            if (!file.getDirectory().getPath().equals(d.getPath())) {
                                if (Uploader.getInstance().moveFile(file, d)) {
                                    file.setDirectory(d);
                                }
                            }
                            this.fileService.update(file);
    ...
    public Set<File> compeleteUploadFromDropZone(String filesTail, String dirPath, Scalr.Method method, Scalr.Mode mode, TtThumnailCropMode cropMode, BufferedImageOp op, TtThumbnailSize... sizes) {
        //  OutLog.pl(filesTail);
        if (filesTail != null && !filesTail.isEmpty()) {
            String s[] = filesTail.split(",");
            Set<File> fls = new HashSet<>();
            ...
            for (String fi : s) {
                try {
                    ...
                    if (file != null) {
                        ...
                        fls.add(file);
                        Uploader.getInstance().thumbnail(file, method, mode, cropMode, op, sizes);
                    }
                } catch (Exception e) {
                }
            }
    ...
    public Set<File> compeleteUploadFromDropZone(String filesTail, String dirPath, TtThumbnailsOf thumbnailsOf) {
        OutLog.pl("dir: " + dirPath);
        return compeleteUploadFromDropZone(filesTail, dirPath, thumbnailsOf.getMethod(), thumbnailsOf.getMode(), thumbnailsOf.getCropMode(), thumbnailsOf.getImageOp(), thumbnailsOf.getThumbs());
    }
