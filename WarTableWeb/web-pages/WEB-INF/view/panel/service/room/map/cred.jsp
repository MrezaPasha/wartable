...
<div class="container">
    <c:choose>
        <c:when test="${room_Map.id!=0}">
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.room.map.edit"/></span>
                <span class="cred-name"><spring:message code="room_Map.roomTitle"/>: [${room.name}]</span> &nbsp;&nbsp;&nbsp;
                <span class="cred-name"><spring:message code="model.id"/>: [${room_Map.id}]</span>
            </h3>
        </c:when>
        <c:otherwise>
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.room.map.create"/></span>
                <span class="cred-name"><spring:message code="room_Map.roomTitle"/>: [${room.name}]</span>
            </h3>
        </c:otherwise>
    </c:choose>
    <div class="tabbable">
        <div class="well well-act">
            ...
            <ul class="disabled-li margin-bottom xs-center-ul-4">
                <li class="tooltips " data-original-title="<spring:message code="all.list"/>" data-placement="bottom">
                    <a href="${_url}/map/list/${room.id}" class="btn btn-default btn-block btn-act">
        ...
        <div class="tab-content">
            <form id="form" accept-charset="UTF-8" action="${action}" method="POST">
                <form:hidden id="room_Map_id" path="room_Map.id"/>
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <c:set var="varName" value="room_Map.room"/>
                        ...
                        <div class="form-group">
                            <c:set var="varName" value="room_Map.map"/>
                            <form:label path="${varName}" cssClass="control-label">
                                <spring:message code="${varName}"/>
                                <span class="symbol required"></span>
                                <form:errors cssClass="form-validation-error" path="${varName}"/>
