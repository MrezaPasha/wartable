    ...
    <c:choose>
        <c:when test="${room_Map.id!=0}">
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.map.room.edit"/></span>
                <span class="cred-name"><spring:message code="room_Map.mapTitle"/>: [${map.name}]</span> &nbsp;&nbsp;&nbsp;
                <span class="cred-name"><spring:message code="model.id"/>: [${room_Map.id}]</span>
            </h3>
        </c:when>
        <c:otherwise>
            <h3 class="cred-header">
                <span class="cred-title"><spring:message code="T.p.service.map.room.create"/></span>
                <span class="cred-name"><spring:message code="room_Map.mapTitle"/>: [${map.name}]</span>
            </h3>
        </c:otherwise>
    </c:choose>
        ...
        <div class="well well-act">
            ...
            <ul class="disabled-li margin-bottom xs-center-ul-4">
                <li class="tooltips " data-original-title="<spring:message code="all.list"/>" data-placement="bottom">
                    <a href="${_url}/room/list/${map.id}" class="btn btn-default btn-block btn-act">
