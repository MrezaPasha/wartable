...
// encapsulation Args
    ...
    var TEMPLATE = {
         ...
         * @desc cleanButton
         */
        cleanButton: "<i class='clean-button clip-close-2' >" + //
        "</i>",
        /**
         * @desc datepicker
         */
        ...
        views: {
            ...
            "default": {
                ...
                render: function (self) {
                    ...
                    self.element.cleanButton = $(TEMPLATE.cleanButton).insertAfter(self.inputElem);
                    ...
                    var pd;
                    /*update datepicker via input value */
                    if (self.inputElem.val()) {
                        pd = self.initPersianDate();
                        self.state.setSelected('unix', pd);
                        self.element.cleanButton.show();
                    } else {
                    ...
                    if (self.fixTime) {
                        var vs = self.fixTime.split(":");
                        self.timePicker._currentMeridian = "";
                        self.timePicker._initState("hour", vs[0]);
                        self.timePicker._initState('minute', vs[1]);
                        self.timePicker._initState('second', vs[2]);
                        self.timePicker.hourInput.val(vs[0]);
                        self.timePicker.minuteInput.val(vs[1]);
                        self.timePicker.secondInput.val(vs[2]);
                    }


                    /*update view of datepicker via input value */
                    if (pd) {
                        self.timePicker._currentMeridian = "";
                        self.timePicker._updateState("hour", pd.hour());
                        self.timePicker._updateState('minute', pd.minute());
                        self.timePicker._updateState('second', pd.second());
                        self.timePicker.hourInput.val(pd.hour());
                        self.timePicker.minuteInput.val(pd.minute());
                        self.timePicker.secondInput.val(pd.second());
                ...
                fixPosition: function (self) {
                    if (!self._inlineView) {
                        ...
                        var inputY = self.inputElem.offset().left + self.inputElem.width() - self.element.main.width();
...
//                var garegurianDate = new Date(this.inputElem.val()).valueOf(); 
    ...
    var ClassTimePicker = {
        ...
        _initState: function (key, val) {
            this.datepicker.initTime(key, val);
            this._updateMeridian(this.datepicker.state);
            return this;
        },
        /**
    ...
    var ClassDatepickerState = {
        ...
        /**
         *
         * @param {type} value  '19:02:50'
         * @returns {Class_DatepickerState}
         */
        setSelectedTime: function (value) {
            var self = this;
            var vs = value.split(":");
            self.selected.hour = vs[0];
            self.selected.minute = vs[1];
            self.selected.second = vs[2];
            self.timePicker._currentMeridian = "";
            self.timePicker._updateState("hour", vs[0]);
            self.timePicker._updateState('minute', vs[1]);
            self.timePicker._updateState('second', vs[2]);
            self.timePicker.hourInput.val(vs[0]);
            self.timePicker.minuteInput.val(vs[1]);
            self.timePicker.secondInput.val(vs[2]);
            self._updateSelectedUnix();
